...
If there are numbers or special characters included in the string, they should be returned as they are. Only letters from the
...
def rot13(message):
    ...
    for char in message:
        if char.isalpha():
            ...
            if (ord(char) > 90 and ord(char) < 104) or (ord(char) > 122 and ord(char) < 136):
                characters.append(chr(ord(char) - 26))
            else:
...
And keep track of the maximum sum contiguous segment among all positive segments (max_so_far is used for this). Each time we get a 
